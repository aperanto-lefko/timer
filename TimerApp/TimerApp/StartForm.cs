...
namespace TimerApp
{
    public partial class TimerStartForm : BaseForm
    {
        ...
        private System.Windows.Forms.Timer fullTimer;  //таймер на полное врем€
        Trainee trainee;
        ...
        private int index = 0;
        private int titleStageTime; //статическое врем€ дл€ промежутков
        ...
        public TimerStartForm()
        {
            ...
            timer.Interval = 1000; // »нтервал 1 секунда
            timer.Tick += Timer_Tick;
            fullTimer = new System.Windows.Forms.Timer();
            fullTimer.Interval = 1000; // »нтервал 1 секунда
        ...
        private void SetFontValueForTitleTimer(string title, int sec)
        {
            ...
            labelTitleTimer.Text = $"{title}:{sec:D2}"; // ‘орматирование: name:SS
        ...
        private void Timer_Tick(object sender, EventArgs e) //задание работы таймера
        {
            if (timeStage > 0)
            {
                ...
                timeStage--;
                if (timeStage >= 1000 && timeStage < 10000) //дл€ 4-х значных значений таймера
                {
            ...
            else
            {
                if (timeStages.Count > 0)
                {
                    ...
                    labelTimer.Text = timeStage.ToString(); // ќбновл€ем текст метки
                    CenterLabelTimer();
                    //задаем значение дл€ названи€ текущего этапа
        ...
        private void AddTrainee()
        {
            if (addTraineeForm.ShowDialog() == DialogResult.OK)
            {
                ...
                if (trainee.RunUpTime != 0)
                {
                    ...
                    titleStages.Enqueue("ѕодготовка");
                }
                ...
                titleStageTime = timeStage; //вспомогательна€ переменна€ хранит длительность текущего этапа тренировки
                //установка начального значени€ дл€ таймера этапов
                ...
                //установка начального значени€ дл€ таймера полного
                SetFontValueForFullTimer(new Font("Comic Sans MS", 40, FontStyle.Bold), sumSeconds);
            }
        ...
        private void SetFontValueForTimer(Font font, int sec)
        {
            ...
            labelTimer.Text = sec.ToString(); // ќбновл€ем текст метки
            CenterLabelTimer();
        }
        private void SetFontValueForFullTimer(Font font, int totalSeconds)
        {
            ...
            labelFullTimer.Text = $"{min:D2}:{sec:D2}"; // ‘орматирование: MM:SS
        ...
        private void musicBut_Click(object sender, EventArgs e)
        {
            ...
            // ѕодписываемс€ на событие закрыти€ формы
            musicForm.FormClosed += (s, args) =>
            {
                // –азблокируем кнопку после закрыти€ формы
